...
export function useDatabase() {
  ...
  async function createSwimmer(data: Omit<Swimmer, 'id'>) {
    const query =
      'INSERT INTO swimmers (name,gender,year_of_birth) VALUES (?,?,?);';
    try {
  ...
  async function updateSwimmer(data: Swimmer) {
    const query =
      'UPDATE swimmers SET name = ?, gender = ?, year_of_birth = ? WHERE id = ?';
    try {
      await database.getAllAsync(query, [
        data.name,
        data.gender,
  ...
  async function getEvent(data: Omit<Event, 'id'>) {
    const query =
      'SELECT id FROM events WHERE distance = ? AND stroke = ? AND course = ?';
    try {
      const response = await database.getFirstAsync<{ id: number }>(query, [
        data.distance,
        data.stroke,
        data.course,
      ]);
  ...
  async function addTime(data: Omit<Time, 'id'>) {
    const query =
      'INSERT INTO times (swimmer_id,event_id,time,date) VALUES (?,?,?,?)';
    try {
      await database.getAllAsync(query, [
        data.swimmer_id,
        data.event_id,
  ...
  async function getSwimmerRelay(
    swimmer_id: number,
    course: string,
    distance: number,
  ) {
    ...
    try {
      const response = database.getAllAsync<SwimmerRelay>(query, [
        swimmer_id,
        course,
        distance,
      ]);
