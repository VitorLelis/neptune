...
import convertEventTimes, {
  EventTimeItem,
  TimeDatePair,
} from '@/utils/eventTimesUtils';
  ...
  const handleActionPress = (index: number) => {
    switch (index) {
      ...
      case 2:
        Alert.alert(
          'Delete Swimmer',
          'This will also delete the swimmer times!',
          [
            {
              text: 'Cancel',
              style: 'cancel',
            },
            {
              ...
              onPress: () => {
                ...
                getSwimmerInfo(), router.navigate('/');
              },
            },
          ],
        );
  ...
  const handleDeleteTime = (time_id: number) => {
    Alert.alert('Delete Time', 'This action cannot be undone!', [
      ...
      },
      {
        text: 'OK',
        onPress: () => {
          deleteTime(time_id), getSwimmerInfo();
        },
      },
    ]);
  };
    ...
    <View style={{ flex: 1, padding: 20 }}>
      ...
      >
        ...
        <Text style={styles.resultText}>
          ...
          {new Date().getFullYear() - data.year_of_birth} (born in{' '}
          {data.year_of_birth})
        </Text>
      </View>
      ...
      >
        ...
        {expandedKey && (
          <FlatList
            data={
              eventList.find(item => item.key === expandedKey)?.events || []
            }
            ...
            renderItem={({ item, index }) => (
              ...
              >
                <Text>
                  ({index + 1}) {timeToString(item.time)} - {item.date}
                </Text>
                <TouchableOpacity
                  onPress={() => handleEditTime(item, expandedKey)}
                >
